...
$.formValidator = 
{
	...
	initConfig : function(controlOptions)
	{
		...
		//window.toActiveElement = document.activeElement;
		if(settings.mode == "SingleTip"){settings.errorFocus=false};settings.errorFocus=true;
		//如果填写了表单和按钮，就注册验证事件
		if(settings.formID!=""){
	...
	setTipState : function(elem,showclass,showmsg)
	{
		...
		else
		{
			...
			var stype = elem.type;
			if(stype == "password" || stype == "text" || stype == "file" || stype=="textarea" || stype=="select-one")
			{
				jqobj = $(elem);
				if(onShowClass!="" && showclass == "onShow"){jqobj.removeClass('input_public input_error input_focus input_correct').addClass(onShowClass)};
				if(onFocusClass!="" && showclass == "onFocus"){jqobj.removeClass('input_public input_error input_focus input_correct').addClass(onFocusClass)};
				if(onCorrectClass!="" && showclass == "onCorrect"){jqobj.removeClass('input_public input_error input_focus input_correct').addClass(onCorrectClass)};
				if(onErrorClass!="" && showclass == "onError"){jqobj.removeClass('input_public input_error input_focus input_correct').addClass(onErrorClass)};
			}
		}
	},
        ...
        switch(sType)
		{
			...
			case "file":
				...
				if (initConfig.wideWord)
				{
					...
					{
						len = len + ((val.charCodeAt(i) >= 0x4e00 && val.charCodeAt(i) <= 0x9fa5) ? 1 : 1); //把汉字也按一个
					}
				}
				else{
					len = val.length;
	...
	pageIsValid : function (validatorGroup)
	{
		...
		else
		{
			...
			window.toActiveElement = $("#" + thefirstid);
		}
	...
	ajaxValid : function(e)
	{
		...
		//$.formValidator.serialize(initConfig.ajaxObjects);
		//添加触发的控件名、随机数、传递的参数
...
$.fn.formValidator = function(cs) 
{
	...
	if(cs.validatorGroup == undefined){cs.validatorGroup = $('form').has($(this)).attr('id')};
	//先合并整个配置(深度拷贝)
	...
	return this.each(function(e)
	{
		...
		{			
			...
			{		
				...
				var offset = $("#"+relativeID ).offset();
				setting_temp.tipCss.top = offset.top;
				setting_temp.tipCss.left = $("#"+relativeID ).width() + offset.left + setting_temp.tipCss.left;
				tipDiv = $("<div style='position:absolute;font-size:12px;' id='"+tip+"'></div>").appendTo($("body"));
